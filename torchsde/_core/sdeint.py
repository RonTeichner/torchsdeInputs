...
def sdeint(sde,
    ...
    sde, y0, ts, bm, method = check_contract(sde, y0, d, ts, bm, method, names, logqp)
    if not noGradCheck:
        misc.assert_no_grad(['ts', 'dt', 'rtol', 'atol', 'dt_min'], [ts, dt, rtol, atol, dt_min])
    return integrate(
        sde=sde,
        y0=y0,
        ts=ts,
...
def check_contract(sde, y0, d, ts, bm, method, names, logqp):
    if names is None:
        names_to_change = {}
    else:
...
def integrate(sde, y0, ts, d, bm, method, dt, adaptive, rtol, atol, dt_min, options, logqp=False):
    if options is None:
        options = {}
